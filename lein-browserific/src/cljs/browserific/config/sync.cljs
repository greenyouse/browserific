...
(defn ^:private tag-and-edn [coll-path path tag-fn id-key tx-data edn-handler-fn]
  (let [tag (if-not (nil? tag-fn)
              (tag-fn tx-data)
              (tx-tag tx-data))
        edn (if-not (nil? edn-handler-fn)
              (edn-handler-fn coll-path path id-key tx-data)
              (condp = tag
...
(defn om-sync
  ...
  :edn-handler-fn - when this option is used, this custom handler
    function will format the edn data for the transaction. The
    handler takes four arguments: the value of :coll, the path
    through the cursor to the updated value, the value of :id-key,
    and the updated state. The output of the handler is the edn data
    sent to the server.
  ...
  provided channel. This allows for higher level operations such as
  server request batching and multiple om-sync component coordination."
  ...
  ([{:keys [url coll] :as data} owner opts]
     ...
     (reify
       ...
       om/IWillMount
       (will-mount [_]
         (let [{:keys [id-key filter tag-fn sync-chan edn-hander-fn]} opts
