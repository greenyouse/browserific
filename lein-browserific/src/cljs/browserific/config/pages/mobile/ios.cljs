...
(defn ios-page
  ...
  (reduce (fn [div c]
    ...
    [{:label "iOS Preferences"
      ...
      :hurl ["https://cordova.apache.org/docs/en/4.0.0/guide_platforms_ios_config.md.html#iOS%20Configuration" "https://cordova.apache.org/docs/en/4.0.0/config_ref_index.md.html#The%20config.xml%20File"]
      :prefs
      [{:type :checkbox :data (reagent/cursor [:mobile :preferences :EnableViewportScale] config-db) :label "Viewport Scale?"
       ...
       {:type :checkbox :data (reagent/cursor [:mobile :preferences :MediaPlaybackRequiresUserAction] config-db) :label "UserAction for Media?"
       ...
       {:type :checkbox :data (reagent/cursor [:mobile :preferences :AllowInlineMediaPlayback] config-db) :label "Inline MediaPlayback?"
       ...
       {:type :select :data (reagent/cursor [:mobile :preferences :BackupWebStorage] config-db) :label "Backup Web Storage"
       ...
       {:type :select :data (reagent/cursor [:mobile :preferences :TopActivityIndicator] config-db) :label "Top Activity Indicator"
       ...
       {:type :checkbox :data (reagent/cursor [:mobile :preferences :KeyboardDisplayRequiresUserAction] config-db) :label "Keyboard UserAction?"
        :help "Set to false to allow the keyboard to appear when calling focus() on form inputs."
        :default true}
       {:type :checkbox :data (reagent/cursor [:mobile :preferences :SuppressesIncrementalRendering] config-db) :label "Supress Incremental Rendering?"
        :help "Set to true to wait until all content has been received before it renders to the screen."}
       {:type :name :data (reagent/cursor [:mobile :preferences :GapBetweenPages] config-db) :label "Gap Between Pages"
        :help "The size of the gap, in points, between pages."}
       {:type :name :data (reagent/cursor [:mobile :preferences :PageLength] config-db) :label "Page Length"
       ...
       {:type :select :data (reagent/cursor [:mobile :preferences :PaginationBreakingMode] config-db) :label "Pagination Breaking Mode"
        ...
        :options ["page" "column"]}
       {:type :select :data (reagent/cursor [:mobile :preferences :PaginationMode] config-db) :label "Pagination Mode"
       ...
       {:type :select :data (reagent/cursor [:mobile :preferences :UIWebViewDecelerationSpeed] config-db) :label "UI WebView Decelartion Speed"
