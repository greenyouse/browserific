...
(defn safari-page
  ...
  (reduce (fn [div c]
    ...
    [:div]
    [;; TODO Create another multi input for this
     #_  {:type :name :data (reagent/cursor [:extensions :extra :safari :bars] config-db) :label "Extension Bars"
     ...
     ;; TODO: another multi input
     #_  {:type :name :data (reagent/cursor [:extensions :extra :safari :context-items] config-db) :label "Context Items"
     ...
     #_  {:type :name :data (reagent/cursor [:extensions :extra :safari :menu] config-db) :label "Menus"
     ...
     ;; TODO: another multi input
     #_  {:type :name :data (reagent/cursor [:extensions :extra :safari :popovers] config-db) :label "Popovers"
          :htxt "Pop-up windows containing HTML content that you associate with a toolbar item you have created."
          :hurl "https://developer.apple.com/library/safari/documentation/Tools/Conceptual/SafariExtensionGuide/UsingExtensionBuilder/UsingExtensionBuilder.html#//apple_ref/doc/uid/TP40009977-CH2-SW1"}
     ;; TODO: another multi input
     #_  {:type :name :data (reagent/cursor [:extensions :extra :safari :toolbar] config-db) :label "Toolbar"
     ...
     {:type :name :data (reagent/cursor [:extensions :extra :safari :start-script] config-db) :label "Start Script"
      ...
      :hurl "https://developer.apple.com/library/safari/documentation/Tools/Conceptual/SafariExtensionGuide/UsingExtensionBuilder/UsingExtensionBuilder.html#//apple_ref/doc/uid/TP40009977-CH2-SW1"}
     ;; TODO: do we need to add regex patters if the choice is some?
