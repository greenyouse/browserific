...
(defn create
  ...
  Doesn't work AmazonFire on Tizen.

  * contact -- a contact object as a clojure map"
  [contact]
  (let [c (clj->js contact)]
    (chenex/ex! [:amazon-fire :tizen] (js/navigator.contacts.create c))))

(comment (throw (js/Error. "Browserific Error: contact/create does not work on amazon-fire or tizen")))
...
(defn finder
  "Searches asynchronously for a contact. The field is used to search for the
   ...
   Doesn't work on AmazonFire or Tizen
  ...
  * opts -- an instance of js/ContactFindOptions()"
  ...
  (let [o-map (first opts)]
    (chenex/ex! [:amazon-fire :tizen]
      (js/navigator.contacts.find field success err o-map))))

(comment (throw (js/Error. "Browserific Error: contact/find does not work on amazon-fire or tizen")))

(defn pick
  "Launches a contact picker to select the contact. The result is passed to the
  callback.

  Doesn't work on AmazonFire, BlackBerry, FirefoxOS, WP 7, or Tizen

  * success -- onSuccess callback gives the Contact object
  * err -- onError callback"
  [success & err]
  (let [e (first err)]
    (chenex/ex! [:amazon-fire :blackberry :firefoxos :wp7 :tizen]
      (js/navigator.contacts.pickContact success e))))

(comment (throw (js/Error. "Browserific Error: contact/pick does not work on amazon-fire, blackberry, firefoxos, wp7 or tizen")))
